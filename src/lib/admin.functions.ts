...
export const adminToggleUserAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.make_admin) {
      ...
      // Safety: don't allow an admin to revoke their own last admin role
...
export const adminUpsertCategory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await ctx.supabase.from("categories").insert(data.data);
    return { error: error?.message ?? null };
  });
...
export const adminUpsertCoupon = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await ctx.supabase.from("coupons").insert(data.data);
    return { error: error?.message ?? null };
  });
...
export const adminDeleteCoupon = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context as any);
    const { error } = await (context as any).supabase.from("coupons").delete().eq("id", data.id);
    return { error: error?.message ?? null };
  });
